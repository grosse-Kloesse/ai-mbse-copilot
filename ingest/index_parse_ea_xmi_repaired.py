...
def should_stub_from_stype(s_type: Optional[str], name: Optional[str]) -> bool:
    ...
    if s_type in {
        ...
        return False
    if not name and s_type in {"Object", "StateNode", "ActionPin"}:
        return False
    return True
...
def ensure_stub_element(
    ...
    if existing is not None:
        if name and (not existing.name or existing.name == existing.id):
            existing.name = name
        if xmi_type and not existing.xmi_type:
            existing.xmi_type = xmi_type
        if package_id and not existing.package_id:
    ...
    def walk_package(pkg_elem: ET.Element, parent_id: Optional[str]) -> None:
        ...
        if pkg is None:
            pkg = PackageRecord(
                ...
                parent_id=parent_id,
                raw=dict(pkg_elem.attrib),
            )
            packages[pkg_id] = pkg
        else:
            pkg.parent_id = parent_id
...
def parse_basic_elements(root: ET.Element, elements: dict[str, ElementRecord], packages: dict[str, PackageRecord]) -> None:
    ...
    def walk_package(pkg_elem: ET.Element, package_id: Optional[str]) -> None:
        for child in pkg_elem.findall("./packagedElement"):
            ...
            if child_type not in element_xmi_types:
                continue
            if not child_id:
                continue
            ...
            if child_id not in elements:
                element = ElementRecord(
                    ...
                    xmi_type=child_type,
                    package_id=package_id,
                    visibility=child.attrib.get("visibility"),
                    raw={"basic_attrs": dict(child.attrib)},
                )
                elements[child_id] = element
            else:
                element = elements[child_id]
                element.package_id = element.package_id or package_id

            if package_id and package_id in packages:
                append_unique(packages[package_id].element_ids, child_id)

            for attr in child.findall("./ownedAttribute"):
                prop_id = attr.attrib.get(f"{{{NS['xmi']}}}id")
                prop_name = attr.attrib.get("name", "")
                ...
                if type_elem is not None:
                    type_ref = (
                        type_elem.attrib.get(f"{{{NS['xmi']}}}idref")
                        or type_elem.attrib.get("href")
                        or type_ref
                    )

                dv = attr.find("./defaultValue")
                if dv is not None:
                    default_value = sanitize_bad_encoding(clean_text(dv.attrib.get("value")))
                ...
                element.properties.append(
                    PropertyRecord(
                        ...
                        raw={"attr_attrs": dict(attr.attrib)},
                    )
                )
...
def parse_diagrams(root: ET.Element, diagrams: dict[str, DiagramRecord], packages: dict[str, PackageRecord]) -> None:
    ...
    for d in diagrams_parent.findall("./diagram"):
        diagram_id = d.attrib.get(f"{{{NS['xmi']}}}id")
        if not diagram_id:
            continue

        model_elem = d.find("./model")
        props_elem = d.find("./properties")

        package_id = None
        if model_elem is not None:
            package_id = model_elem.attrib.get("package") or model_elem.attrib.get("package2")

        name = props_elem.attrib.get("name", diagram_id) if props_elem is not None else diagram_id
        diagram_type = props_elem.attrib.get("type") if props_elem is not None else None
        ...
        diagram = DiagramRecord(
            ...
            raw={
                ...
                "model_attrs": dict(model_elem.attrib) if model_elem is not None else {},
                "properties_attrs": dict(props_elem.attrib) if props_elem is not None else {},
            },
        )
        diagrams[diagram_id] = diagram

        if package_id and package_id in packages:
            append_unique(packages[package_id].diagram_ids, diagram_id)

        elem_parent = d.find("./elements")
        if elem_parent is None:
            continue

        for view_elem in elem_parent.findall("./element"):
            subject_id = view_elem.attrib.get("subject")
            if not subject_id:
                continue

            diagram.element_views.append(
                ElementView(
                    diagram_id=diagram_id,
                    element_id=subject_id,
                    view_kind="direct",
                    geometry=view_elem.attrib.get("geometry"),
                    style=view_elem.attrib.get("style"),
                    seqno=view_elem.attrib.get("seqno"),
                    raw={"view_attrs": dict(view_elem.attrib)},
                )
            )


def parse_sysml_semantics(root: ET.Element, elements: dict[str, ElementRecord], relations: dict[str, RelationRecord]) -> None:
    for req in root.findall(".//SysML:requirement", NS):
        element_id = req.attrib.get("base_Class")
        if not element_id or element_id not in elements:
            continue
        elements[element_id].semantic_type = "Requirement"
        elements[element_id].source_id = req.attrib.get("id")
        elements[element_id].text = sanitize_bad_encoding(clean_text(req.attrib.get("text")))
        elements[element_id].stereotype = "requirement"

    for blk in root.findall(".//SysML:block", NS):
        element_id = blk.attrib.get("base_Class")
        if not element_id or element_id not in elements:
            continue
        elements[element_id].semantic_type = "Block"
        elements[element_id].stereotype = "block"

    for vt in root.findall(".//SysML:ValueType", NS):
        element_id = vt.attrib.get("base_DataType")
        if not element_id or element_id not in elements:
            continue
        elements[element_id].semantic_type = "ValueType"
        if vt.attrib.get("unit"):
            elements[element_id].custom["unit_ref"] = vt.attrib.get("unit")
        if vt.attrib.get("quantityKind"):
            elements[element_id].custom["quantity_kind"] = vt.attrib.get("quantityKind")

    for sat in root.findall(".//SysML:satisfy", NS):
        rel_id = sat.attrib.get("base_Dependency")
        if not rel_id:
            continue
        ...
        rel.semantic_type = "satisfy"
        rel.stereotype = rel.stereotype or "satisfy"
...
def parse_extension_elements(root: ET.Element, elements: dict[str, ElementRecord]) -> None:
    ...
    elements_parent = ext.find("./elements")
    if elements_parent is None:
        return

    for elem in elements_parent.findall("./element"):
        element_id = elem.attrib.get(f"{{{NS['xmi']}}}idref")
        if not element_id:
            continue

        props = elem.find("./properties")
        project = elem.find("./project")
        ext_props = elem.find("./extendedProperties")
        model_node = elem.find("./model")
        tags_parent = elem.find("./tags")

        s_type = props.attrib.get("sType") if props is not None else None
        inferred_xmi_type = infer_xmi_type_from_stype(s_type)
        package_id = model_node.attrib.get("package") if model_node is not None else None
        name = elem.attrib.get("name", "")

        if element_id not in elements and should_stub_from_stype(s_type, name):
            ensure_stub_element(
                elements=elements,
                element_id=element_id,
                name=name or element_id,
                xmi_type=inferred_xmi_type,
                package_id=package_id,
            )
        ...
        if inferred_xmi_type and not record.xmi_type:
            record.xmi_type = inferred_xmi_type
        if package_id and not record.package_id:
            record.package_id = package_id
        if name and (not record.name or record.name == record.id):
            record.name = name

        if props is not None:
            record.stereotype = record.stereotype or props.attrib.get("stereotype")
            record.custom["sType"] = props.attrib.get("sType")

        if ext_props is not None:
            record.custom["package_name"] = ext_props.attrib.get("package_name")
...
def link_refs(
    ...
    for diagram in diagrams.values():
        ...
        if package_subject_ids:
            diagram.raw["resolved_package_subjects"] = list(dict.fromkeys(package_subject_ids))
        if property_subject_ids:
            diagram.raw["resolved_property_subjects"] = list(dict.fromkeys(property_subject_ids))
        if ignored_container_subjects:
            diagram.raw["ignored_view_subjects"] = list(dict.fromkeys(ignored_container_subjects))
...
def build_relation_chunk_rows(
    ...
    for r in relations.values():
        ...
        rows.append({
            ...
            "source_id": r.source_id,
            "target_id": r.target_id,
            "source_name": normalize_display_name(source_name),
            "target_name": normalize_display_name(target_name),
            "type": r_type,
            "text": "\n".join(parts),
        })
